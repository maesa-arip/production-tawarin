...
export default function Team({ setIsOpenDialog, model }) {
    ...
    const dateMatches = (dateString1, dateString2) => {
        ...
        console.log(`Comparing dates: ${formattedDate1} and ${formattedDate2}`);
    ...
    const timeRangeMatches = (timeRange1, timeRange2) => {
        const [start1, end1] = timeRange1.split(" - ").map(t => new Date(`2000-01-01T${t}:00`));
        const [start2, end2] = timeRange2.split(" - ").map(t => new Date(`2000-01-01T${t}:00`));
        return start1 < end2 && end1 > start2;
    };

    const userWorkBreakMatches = (userId, timeRange) => {
        return model2.workBreak.some(
                            ...
                            <div className="grid w-full grid-cols-12 gap-2">
                                <div className="col-span-12 mx-2">
                                    <p className="flex items-start">
                                        {resultteam.name} {resultteam.user_id}
                                    </p>
                                </div>
