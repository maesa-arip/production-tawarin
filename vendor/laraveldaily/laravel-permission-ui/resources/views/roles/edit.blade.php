...
@section('content')
    <form class="space-y-4" action="{{ route(config('permission_ui.route_name_prefix') . 'roles.update', $role) }}" method="post">
        ...
        <div class="space-y-2">
            ...
            <input class="block w-full px-2 py-1 border border-gray-300 rounded-md shadow-sm" type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required />
            @error('name')
                ...
                <div class="space-x-2">
                    @foreach($permissions as $id => $name)
                        <div class="inline-flex space-x-1">
                            <input class="border-gray-300 rounded-md shadow-sm" type="checkbox" name="permissions[]" id="permission-{{ $id }}" value="{{ $id }}" @checked(in_array($id, old('permissions', [])) || $role->permissions->contains($id))>
